...
# 3. compute and output the normalized adjacency matrix, referring to Equation 1 in the paper.
    ...
    Degree1 = Degree + AdjIden    #self-loop
    Adj1 = Adj + AdjIden
    DegreePower = fractional_matrix_power(Degree1, -0.5)
    AdjNorm = np.matmul(np.matmul(DegreePower, Adj1), DegreePower)    #D(-1/2)AD(-1/2)
...
def MakeGraph_EMG():
# This function is used for the situation where the target graph can be easily & manually defined.
# Here is an example how we define the graph of muscles on back with 4 nodes. 4 nodes are fully connected
# 1. define the 4X4 adjacency matrix .
    Adj = np.zeros((4, 4))
    Adj[0, 1] = 1
    Adj[0, 2] = 1
    Adj[0, 3] = 1
...
# 2. define the diagonal 4X4 degree matrix.
    Degree = np.zeros((4, 4))
    Degree[0, 0] = 3
    Degree[1, 1] = 3
    Degree[2, 2] = 3
...
def combine_mocap(Adj, X, time_step, node_num, feature_num):
...
def combine_emg(Adj, X, time_step, node_num, feature_num):
# Transfer the input matrix X (NxTxD) into graph sequences (N x T x node_num x feature_num) as input
# assume X=[EMG_signal].
# Adj is Adjacency matrix of (None, node_num, node_num).
# time_step is the length of each data segment.
# node_num is the number of nodes/joints, feature_num is the feature dimension per node/joint which is 1 with EMG signal.
    EMG_signal = X[:, :, 0:node_num]
    num_sample, _, _ = X.shape

    nodefeature = np.reshape(EMG_signal, (num_sample, time_step, node_num, 1))
