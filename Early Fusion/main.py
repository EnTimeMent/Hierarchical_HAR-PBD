...
def normalization(mat_EMG):
    def zero_mean(arr):
        ...
        return arr
    #mat_EMG(N,4)
    m = mat_EMG.shape[1]    #number of muscles, zero-mean within each column
    for j in range(m):
        mat_EMG[:,j] = zero_mean(mat_EMG[:,j])
    
    return mat_EMG

#Rebuild Matrix
#treat EMG data as the graph feature, same value of EMG nodes for both dimensions
def new_mat(mat):
    #(N,78) to (N,86)
    #[:78] are X, [78:] are Y
    n = mat.shape[0]
    new_mat = np.zeros((n,86))    #build a new matrix
    mat_EMG = normalization(mat[:,66:70])
    new_mat[:,:22] = mat[:,:22]
    new_mat[:,22] = mat_EMG[:,0]
    new_mat[:,23] = mat_EMG[:,1]
    new_mat[:,24] = mat_EMG[:,2]
    new_mat[:,25] = mat_EMG[:,3]
    
    new_mat[:,26:48] = mat[:,22:44]
    new_mat[:,48] = mat_EMG[:,0]
    new_mat[:,49] = mat_EMG[:,1]
    new_mat[:,50] = mat_EMG[:,2]
    new_mat[:,51] = mat_EMG[:,3]
    
    new_mat[:,52:74] = mat[:,44:66]
    new_mat[:,74] = mat_EMG[:,0]
    new_mat[:,75] = mat_EMG[:,1]
    new_mat[:,76] = mat_EMG[:,2]
    new_mat[:,77] = mat_EMG[:,3]
...
def segmentation(mat,overlap_ratio,window_len):
    ...
    while (m-length)>window_len:
        x_segment = np.concatenate((x_segment,mat[num_frames*step:num_frames*step+window_len,:78]),0)
        
        y_HAR_segment = int(Counter(mat[num_frames*step:num_frames*step+window_len,78]).most_common(1)[0][0])
        ...
        if y_HAR_segment==6 or y_HAR_segment==7 or y_HAR_segment==8:
            y_HAR_segment = 0 
        y_PDB_segment = int(Counter(mat[num_frames*step:num_frames*step+window_len,80]).most_common(1)[0][0])
    ...
    #final concatenation
    x_segment = np.concatenate((x_segment,mat_last[:,:78]),0)
    y_HAR_segment = int(Counter(mat_last[:,78]).most_common(1)[0][0])
    if y_HAR_segment==6 or y_HAR_segment==7 or y_HAR_segment==8:
            y_HAR_segment = 0 
    y_PDB_segment = int(Counter(mat_last[:,80]).most_common(1)[0][0])
    y_segment = np.concatenate((y_segment,[[y_HAR_segment,y_PDB_segment]]),0)
    
    #reshape the data
    num_frames = num_frames + 1
    x_segment = np.reshape(x_segment[1:],(num_frames,window_len,78))
...
model, HARmodel = HierarchicalHAR_PBD.build_model(180,26,3,26,24,16,24,6,2)
